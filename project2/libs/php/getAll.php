<?php
	...
	$query = 'SELECT p.id AS personnelId, p.lastName, p.firstName, p.jobTitle, p.email, d.id AS departmentId, d.name as departmentName, l.name as location FROM personnel p LEFT JOIN department d ON (d.id = p.departmentID) LEFT JOIN location l ON (l.id = d.locationID) ORDER BY p.lastName, p.firstName, d.name, l.name';
